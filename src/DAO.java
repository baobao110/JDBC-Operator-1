...
public class DAO {
	public void add(Node node) {//这里利用分层的思想，将数据类中的数据封装在Node类型的变量中
		String sql="insert text (title,time,weather,context)values('"+node.getTitle()+"', NOW(),'"+node.getWeather()+"','"+node.getContext()+"');";
		/*
		 ...
		 */
		try {
			DATA.update(sql);//sql语句拼接完成后直接调用DATA的静态方法
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}
	public void delete(int id) {//进行删除操作时因为在设计数据库时id作为主键，可以直接利用id号进行删除，非常的方便
	...
	/*public static void main(String[] args) {
		DAO dao=new DAO();
		try {
			ArrayList<Node> b=dao.CheckTitleMore("人");
			for(Node i:b) {
